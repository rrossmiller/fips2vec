def fips2vec(
    dimensions, force_train=False, vizualize=False, workers=6, file_name="fips2vec"
):
    import os
    ...
    if not os.path.exists("fips2vec.kv") or force_train:
        ...
        node2vec = Node2Vec(
            g, dimensions=dimensions, walk_length=30, num_walks=200, workers=workers
        )
        ...
        if vizualize:
            print(model.wv["55133"])
            print(model.wv.most_similar("55133"))
...
def write_tsv(dimensions, file_path="."):
    ...
    for i in range(len(file)):
        ...
        labels.append(
            f"{splits[0]}\t{names_df.loc[names_df.num == int(splits[0])].name.iloc[0]}\n"
        )
        vecs.append(splits[1:])

    with open(f"{file_path}/fips_vecs-{dimensions}.tsv", "w") as fout:
